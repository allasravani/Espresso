...
function loadLang(lang){
    $.getJSON("D:/CRM Plugin codes/ESPRESSO/wwwroot/ConferenceCaptioningWeb/wetech/" + lang + ".json", (text) => {
      localization = text
...
function getCount() {
    ...
    // Fetch the access URL on page load
    fetch('https://localhost:7027/api/stream/GetAccessUrl', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
        body: JSON.stringify({ url: currentUrl })
        // If you need to send data in the case of a POST request:
